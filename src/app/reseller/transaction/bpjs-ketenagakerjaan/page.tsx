...
export default function BpjsKetenagakerjaanPage() {
  ...
  const checkBill = () => {
    if (customerId.length !== 11) return toast.error('Nomor Kepesertaan harus 11 digit')
    setLoadingInfo(true)
    ...
    setTimeout(() => {
      if (customerId.startsWith('1')) {
        setBillData({
          name: 'BUDI SANTOSO',
          id: customerId,
          program: 'JHT, JP, JKK, JKM',
          period: '1 Bulan (Juni 2026)',
          billAmount: 185000,
          penalty: 0,
          adminFee: 2500,
        })
      } else {
        toast.error('Data peserta tidak ditemukan atau iuran sudah lunas.')
        setBillData(null)
      }
  ...
  if (receiptTx) {
    return (
      <TransactionReceipt
        ...
        productIcon="👷"
        details={[
          { label: 'No. Kepesertaan', value: customerId },
          { label: 'Nama Peserta', value: billData?.name || '-' },
          { label: 'Program', value: billData?.program || '-' },
          { label: 'Periode', value: billData?.period || '-' },
        ]}
  ...
  const modalData: ConfirmationModalData | null = billData ? {
    productName: 'Iuran BPJS Ketenagakerjaan',
    productIcon: '👷',
    details: [
      { label: 'No. Peserta', value: customerId, highlight: true },
  ...
  return (
    <div className="pb-24">
      <PageHeader title="BPJS Ketenagakerjaan" />

      {/* Input Section */}
      <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 mb-6 mx-4 relative">
        <label className="block text-sm font-bold text-gray-700 mb-2">Nomor Kepesertaan (11 Digit)</label>
        <div className="relative flex items-center mb-4">
          ...
          <input 
            type="tel"
            value={customerId}
            onChange={(e) => { setCustomerId(e.target.value.replace(/\D/g, '').substring(0, 11)); setBillData(null) }}
            placeholder="Nomor Peserta BPJS TK"
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-right w-1/2">{billData.program}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Iuran Pokok</span>
