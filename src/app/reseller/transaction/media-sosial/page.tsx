...
export default function MediaSosialPage() {
  ...
  useEffect(() => {
    api.get('/reseller/products?category=MEDIA_SOSIAL')
  ...
  const filteredProducts = useMemo(() => {
    if (!opName) return []
    let list = products.filter(p => p.name.toLowerCase().includes(opName.toLowerCase()))
    
    if (activeTab !== 'all') {
      list = list.filter(p => p.name.toLowerCase().includes(activeTab.toLowerCase()))
    }
  ...
  if (loading) return <Loading text="Memuat produk sosmed..." />
    ...
    return (
      <TransactionReceipt
        ...
        details={[
          ...
          { label: 'Kategori', value: 'Paket Media Sosial' }
        ]}
  ...
  return (
    <div className="pb-24">
      <PageHeader title="Paket Media Sosial" />

      {/* Input Section */}
      <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 mb-4 mx-4 relative">
      ...
      {opName && filteredProducts.length > 0 ? (
        <div className="px-4 space-y-3 mb-8">
          <h3 className="text-sm font-bold text-gray-900 mb-4 px-1">Pilih Paket Sosmed</h3>
      ...
      ) : phone.length >= 4 && opName ? (
        <div className="text-center p-6 bg-white rounded-3xl border border-gray-100 border-dashed mx-4">
          <p className="text-gray-500 text-sm">Belum ada produk sosmed untuk filter ini.</p>
        </div>
      ) : null}
