...
export default function PerpanjanganMasaAktifPage() {
  ...
  useEffect(() => {
    api.get('/reseller/products?category=MASA_AKTIF')
  ...
  if (loading) return <Loading text="Memuat produk masa aktif..." />
    ...
    return (
      <TransactionReceipt
        ...
        details={[
          ...
          { label: 'Kategori', value: 'Perpanjangan Masa Aktif' }
        ]}
    ...
    <div className="pb-24">
      ...
      {opName && filteredProducts.length > 0 ? (
        <div className="px-4 space-y-3 mb-8">
          <h3 className="text-sm font-bold text-gray-900 mb-4 px-1">Pilih Tambahan Masa Aktif</h3>
          {filteredProducts.map((p, idx) => {
            const isSelected = selectedProduct?.id === p.id
            // Extract duration like "+30 Hari" from name if possible
            const durationMatch = p.name.match(/\+?\d+\s*Hari/i)
              ...
              >
                <div className="flex-1 pr-4">
                  <div className="flex items-center gap-2 mb-1">
                    <Clock className="w-4 h-4 text-[#00B4A0]" />
                    <p className="font-bold text-sm text-gray-900">{duration || p.name}</p>
                  </div>
                  {!durationMatch && <p className="text-xs text-gray-500 mb-2">{p.name}</p>}
                  <p className="font-black text-lg text-[#00B4A0] mt-1">{formatRupiah(p.sellPrice)}</p>
                </div>
                <div>
