...
export default function GasPage() {
  ...
  const checkBill = () => {
    if (customerId.length < 5) return toast.error('ID Pelanggan tidak valid')
    setLoadingInfo(true)
  ...
  if (receiptTx) {
    return (
      <TransactionReceipt
        ...
        productName="Tagihan Gas Negara"
        productIcon="🔥"
        details={[
  ...
  const modalData: ConfirmationModalData | null = billData ? {
    productName: 'Tagihan Gas Negara (PGN)',
    productIcon: '🔥',
    details: [
  ...
  return (
    <div className="pb-24">
      <PageHeader title="Gas Negara (PGN)" />

      {/* Input Section */}
      <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 mb-6">
        <label className="block text-sm font-bold text-gray-700 mb-2">No. Pelanggan PGN</label>
        ...
        <div className="px-4 mb-8">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Pemakaian Gas</span>
      ...
      <div className={`fixed bottom-[70px] left-0 right-0 p-4 transition-transform duration-300 ${billData ? 'translate-y-0' : 'translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="max-w-md mx-auto bg-white border border-red-100 shadow-[0_-10px_40px_-15px_rgba(239,68,68,0.2)] rounded-2xl p-4 flex items-center justify-between">
          ...
          <button 
            ...
            disabled={total === 0}
            className="px-6 py-2.5 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl shadow-lg shadow-red-500/30 transition-colors"
          >
            Lanjut Bayar
          </button>
        </div>
