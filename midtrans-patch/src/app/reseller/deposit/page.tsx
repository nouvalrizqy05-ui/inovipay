...
export default function DepositPage() {
  ...
  async function handleMidtransPay(e: React.FormEvent) {
    e.preventDefault()
    if (Number(form.amount) < 10000) { toast.error('Minimal deposit Rp 10.000'); return }
    if (!selMethod) { toast.error('Pilih metode deposit dulu'); return }
    if (!window.snap) { toast.error('Payment widget belum siap, coba lagi sebentar'); return }

    setSubmitting(true)
    try {
      const res = await api.post('/deposits/midtrans', { amount: Number(form.amount), category: selMethod })
      const { token } = res.data

      window.snap.pay(token, {
        onSuccess: () => {
          toast.success('Pembayaran berhasil! Saldo akan masuk otomatis dalam beberapa saat.')
          setForm({ amount: '', proofUrl: '', note: '' })
          setSelMethod(null)
          fetchDeposits()
        },
        onPending: () => {
          toast.info('Menunggu pembayaran kamu. Saldo masuk otomatis setelah pembayaran dikonfirmasi.')
          fetchDeposits()
        },
        onError: () => {
        ...
        onClose: () => {
          toast.info('Kamu menutup jendela pembayaran sebelum selesai.')
        },
      })
    } catch (err: any) {
      toast.error(err.response?.data?.error ?? 'Gagal membuat transaksi pembayaran')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Deposit otomatis via Midtrans Snap — VA, QRIS, ShopeePay, Indomaret */}
      {selMethod && selMethod !== 'BANK' && (
        <div className="card">
          <p className="font-bold text-sm text-gray-900 mb-3">Detail Deposit (Otomatis)</p>
          <form onSubmit={handleMidtransPay} className="space-y-3">
            <div>
              <label className="label">Jumlah Deposit (Rp)</label>
              <input type="number" className="input text-lg font-bold" placeholder="Minimal Rp 10.000"
                min="10000" step="10000" value={form.amount}
            ...
            <p className="text-xs text-teal-700 bg-teal-50 p-2 rounded-lg">
              💡 Setelah klik bayar, jendela pembayaran Midtrans akan muncul. Saldo masuk otomatis begitu pembayaran dikonfirmasi — tidak perlu upload bukti.
            </p>
            <button type="submit" disabled={submitting} className="btn-primary w-full py-3">
              {submitting ? 'Menyiapkan pembayaran...' : `Bayar ${form.amount ? formatRupiah(Number(form.amount)) : ''} Sekarang`}
            </button>
          </form>
        </div>
      )}
